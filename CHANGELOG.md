...
- Gemini replay payloads are validated against the SDK's own `genai.Part` struct, so a payload that is not a part is rejected pre-flight instead of decoding into an empty part and shipping as `{}`.
