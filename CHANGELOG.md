...
- `agent.Transcript.SaveToFile` and `SaveToFileJSONL` now write at mode `0600` (owner read/write only) instead of following the process umask (typically `~0644`), and replace the destination atomically instead of truncating it in place. The mode is a side effect of fixing kata jh1c — a serialization or write failure no longer destroys the previous transcript — via `os.CreateTemp`, whose temp files are created at `0600`; that mode was kept rather than widened back to the prior umask-derived value. Kata 0xem holds the formal owner-only-permissions approval gate for this file family and remains open; this entry makes the already-shipped behavior change visible and reversible pending that decision.
