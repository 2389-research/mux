...
- Anthropic and Gemini requests now honor a replay envelope instead of ignoring it, and an envelope belonging to a different provider or model is dropped with a warning on stderr naming both identities rather than failing the request (9jn5). Only signature-bearing blocks are stamped — Anthropic `thinking` and `redacted_thinking`, Gemini parts with a `thoughtSignature` — so a history with no thinking in it still switches models freely, and caller edits to assistant text or tool input still reach the wire. Dropping an envelope costs that block its signature: a block with a normalized form, such as a tool call, still reaches the wire without it, while a thinking or provider-item-only block contributes nothing to the request at all. Editing tool input is not free either: Anthropic invalidates the signature on every thinking block after a changed `tool_use`, so an edit mid-conversation means dropping the thinking blocks that follow it. Structural problems in a payload — empty data, invalid JSON, no type discriminator — have no safe normalized fallback and do still fail preflight with zero HTTP requests. The exported `*llm.ErrReplayMismatch` type is removed along with the rejection it described: a mismatch can no longer produce it, so keeping it would be dead surface whose doc comment states the opposite of what the package does. Callers that used `errors.As` with `*llm.ErrReplayMismatch` to detect a model switch must delete that branch — the call now succeeds with a warning on stderr, and there is no error to detect. Dropping happens on a sanitized copy, so a `*Request` is not modified and stays safe to send again: a stale envelope warns on every attempt rather than turning the second attempt into a structural failure. This matters for `llm.RetryClient`, which re-uses the caller's `*Request` on each retry.
